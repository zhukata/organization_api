...
def create_organization(db: Session, organization: OrganizationCreate, phone_numbers: List[str], activity_ids: List[int]):
...
def get_organizations_by_activity(db: Session, activity_id: int):
    ...
    for act_id in all_activity_ids:
        orgs = db.query(Organization).join(Organization.activities).filter(Activity.id == act_id).all()
